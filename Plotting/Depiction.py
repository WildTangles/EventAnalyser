...
class MainDepiction(Depiction):
    ...
    def drawDepiction(self, paintables):
        self.pad.cd()
        if "log_y" in DB.histoptions: self.pad.SetLogy(bool(DB.histoptions["log_y"]))
        ...
        for paintable in paintablesToPaint:
            paintable.setBounds(0.001, maximum)
            if "xndiv" in DB.histoptions:
                paintable.getHistogram().GetXaxis().SetNdivisions(int(DB.histoptions["xndiv"]))
...
class RatioDepiction(Depiction):
    ...
    def drawRatioHistogram(self):
        ...
        self.ratioHistogram.GetYaxis().CenterTitle()        
        self.ratioHistogram.SetTitle("")
        if "xndiv" in DB.histoptions:
            self.ratioHistogram.GetXaxis().SetNdivisions(int(DB.histoptions["xndiv"]))
    ...
    def setBounds(self):
        ...
        counter = 0
        for i in range(self.ratioHistogram.GetNbinsX()):
            if self.ratioHistogram.GetBinContent(i) != 0: counter += 1
        meanvalue = self.ratioHistogram.Integral()/counter
        
        histmax = 0
        for i in range(self.ratioHistogram.GetNbinsX()):
            val = self.ratioHistogram.GetBinContent(i)
            if val > histmax and val < meanvalue*5:
                histmax = val
            
        self.ratioHistogram.SetMaximum(histmax*1.5)
        self.ratioHistogram.SetMinimum(0.000001)
...
class AgreementDepiction(RatioDepiction):
    ...
    def setBounds(self):
        self.ratioHistogram.SetMaximum(1.999)
        self.ratioHistogram.SetMinimum(0.001)
